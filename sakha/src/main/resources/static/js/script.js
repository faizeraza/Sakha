...
function showCreateTaskOptions() {
  addMessage(
    "You chose to create a new task. Please provide the task details.",
    "bot"
  );
...
async function executeTask(taskname) {
  try {
    const response = await fetch(
      `http://localhost:8080/tasks/execute?taskname=${taskname}`
    );
...
async function createTask() {
  ...
  try {
    ...
    addMessage(
      `Task "${createdTask.taskname}" created successfully. Executing now...`,
      "bot"
    );
  } catch (error) {
...
async function createAndExecuteTask() {
  ...
  try {
    ...
    const executeResponse = await fetch(
      `http://localhost:8080/tasks/execute?taskname=${taskData.taskname}`,
      {
        method: "GET",
      }
    );
